...
(async () => {
    ...
    for (var i = 0; i < atos.length; i++) {
        ...
        ato.nrAto = header[3].replace(',', '');
        let dtDay = header[5];
        let dtMonth = header[7].toLowerCase();
        let dtYear = header[9];
    ...
    await fs.writeFileSync("./rhnet/lei-complementar/data/finalResults.json", JSON.stringify({ data: final }, null, 4));
})();