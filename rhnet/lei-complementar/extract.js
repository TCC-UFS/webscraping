...
(async () => {
    ...
    await page.evaluate(async () => {
        ...
        try {
            ...
            log("Select 'Lei Complementar'");
            await document.querySelector("div#accordion-2").children[0].children[2].children[0].click();
            ...
            await document.querySelectorAll("a.p-item-link")[0].click();
            await wait(2);
            ...
            for(var i = 0; i < parseInt(total); i++) {
                try {
                    ...
                    let current = (i+1).toLocaleString("en-us", { minimumIntegerDigits: 4 }).split(',').join('');
            ...
            log(`\nApplication Crashed!\n${err.message}`);
            if (atos.length > 1) {
                await writeFile("./rhnet/lei-complementar/data/backup.json", JSON.stringify({ errors, atos }, null, 4));
            }
            throw err;
        }
    })
    .then(async response => {
        ...
        await fs.writeFileSync('./rhnet/lei-complementar/data/results.json', resultsJSON);
        await fs.writeFileSync('./rhnet/lei-complementar/data/errors.json', errorsJSON);
    })
    .catch(err => {
        throw err;
    });
