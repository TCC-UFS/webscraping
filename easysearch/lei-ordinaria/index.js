...
(async () => {
    ...
    await page.evaluate(async () => {
        ...
        try {
            ...
            log(`\nApplication Crashed!\n${err.message}`);
            await writeFile("./easysearch/lei-ordinaria/backup.json", JSON.stringify({ errors, pageResults }, null, 4));
            throw err;
        }
    })
    .then(async response => {
        ...
        await fs.writeFileSync('./easysearch/lei-ordinaria/results.json', resultsJSON);
        await fs.writeFileSync('./easysearch/lei-ordinaria/errors.json', errorsJSON);
    })
    .catch(err => {
        throw err;
    });
